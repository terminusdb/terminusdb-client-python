...
def test_create_database(mocked_requests):
    ...
    send_dispatch_request(full_url, APIEndpointConst.CREATE_DATABASE, "mykey", payload)
...
@mock.patch("requests.get", side_effect=mocked_requests)
def test_get_schema(mocked_requests):
...
@mock.patch("requests.get", side_effect=mocked_requests)
def test_woql_select(mocked_requests):
    payload = {
    ...
    full_url = "http://localhost:6363/myFirstTerminusDB/woql"
    send_dispatch_request(full_url, APIEndpointConst.WOQL_SELECT, "mykey", payload)
    requests.get.assert_called_once_with(
...
def test_delete_database(mocked_requests):
    full_url = "http://localhost:6363/myFirstTerminusDB"
    send_dispatch_request(full_url, APIEndpointConst.DELETE_DATABASE, "mykey")
