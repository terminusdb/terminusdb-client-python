        ...
        def localized_fn(query=None):
            ...
            for param_name in param_names:
                ...
                if outer_value is not None:
                    ...
                    if isinstance(outer_value, str) and outer_value.startswith('v:'):
                        outer_eq_bindings.append(WOQLQuery().eq(outer_value, outer_value))
                    elif isinstance(outer_value, Var):
                        outer_eq_bindings.append(WOQLQuery().eq(outer_value, outer_value))
                    # Bind the unique variable to the outer parameter OUTSIDE the select("")
                    outer_eq_bindings.append(WOQLQuery().eq(getattr(v, param_name), outer_value))
            ...
            class FluentWrapper(WOQLQuery):
                ...
                def woql_and(wrapper_self, *args):
                    ...
                    if wrapper_self._outer_eq:
                        return WOQLQuery().woql_and(*wrapper_self._outer_eq, localized_query)
                    return localized_query
...
class WOQLLibrary:
    ...
    def rdflist_peek(self, cons_subject, value_var):
        ...
        """
        (localized, v) = WOQLQuery().localize({'consSubject': cons_subject, 'valueVar': value_var})
    ...
    def rdflist_nth1(self, cons_subject, index, value_var):
        ...
        if isinstance(index, int):
            ...
            return localized(self._rdflist_nth0_raw(v.consSubject, index - 1, v.valueVar))
        
        raise ValueError('rdflist_nth1 with variable index not yet supported with localize pattern')
    ...
    def rdflist_length(self, cons_subject, length_var):
        ...
        return localized(self._rdflist_length_raw(v.consSubject, v.lengthVar, v.length_path))
    ...
    def rdflist_slice(self, cons_subject, start, end, result_var):
        ...
        if start == 0 and end == 1:
            ...
            find_nodes = WOQLQuery().path(v.consSubject, f'rdf:rest{{0,{end - 1}}}', v.node)
        else:
            find_nodes = WOQLQuery().path(v.consSubject, f'rdf:rest{{{start},{end - 1}}}', v.node)
        ...
        return localized(
            WOQLQuery().group_by(
                ...
                WOQLQuery().woql_and(
                    find_nodes,
                    WOQLQuery().triple(v.node, 'rdf:first', v.value)
                )
            )
        )
    ...
    def rdflist_insert(self, cons_subject, position, value, new_node_var=None):
        ...
        if position == 0:
            ...
            return localized(
                WOQLQuery().woql_and(
                    WOQLQuery().triple(v.consSubject, 'rdf:first', v.old_first),
                    WOQLQuery().triple(v.consSubject, 'rdf:rest', v.old_rest),
                    WOQLQuery().idgen('list_node', [v.old_first, v.consSubject], v.newNodeVar),
        ...
        path_pattern = '' if rest_count == 0 else f'rdf:rest{{{rest_count},{rest_count}}}'
        
        if rest_count == 0:
            find_predecessor = WOQLQuery().eq(v.pred_node, v.consSubject)
        else:
            find_predecessor = WOQLQuery().path(v.consSubject, path_pattern, v.pred_node)
    ...
    def rdflist_swap(self, cons_subject, pos_a, pos_b):
        ...
        if pos_a == 0:
            ...
            find_node_a = WOQLQuery().path(v.consSubject, f'rdf:rest{{{pos_a},{pos_a}}}', v.node_a)
        
        # Find node at pos_b
        if pos_b == 0:
            find_node_b = WOQLQuery().eq(v.node_b, v.consSubject)
        else:
            find_node_b = WOQLQuery().path(v.consSubject, f'rdf:rest{{{pos_b},{pos_b}}}', v.node_b)
