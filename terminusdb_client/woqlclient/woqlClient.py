...
class WOQLClient:
   ...
   "doc": f"terminusdb://{accountid}/{dbid}/data/"
    ...
    def query(
        self, woql_query, commit_msg = None, file_list=None
    ):
        if(hasattr(woql_query, "_contains_update_check") and woql_query._contains_update_check()):
            if(not commit_msg):
                commit_msg = "Update Query generated by python client"
        ...
        if not woql_query.get('@context'):
            woql_query['@context'] = self.conCapabilities.get_json_context()
