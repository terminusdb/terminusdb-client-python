...
def mocked_requests_get(*args, **kwargs):
    ...
    if args[0] == 'http://localhost:6363/':
        return MockResponse._mock_response()
    #elif args[0] == 'http://someotherurl.com/anothertest.json':
        #return MockResponse({"key2": "value2"}, 200)

    return MockResponse._mock_response(200)

# Our test case class
class dispatchRequestTests(mocker):

    # We patch 'requests.get' with our own method. The mock object is passed in to our test case method.
    @mocker.patch('dispatchRequest.requests.get', side_effect=mocked_requests_get)
    def test_connectCall(self, mock_get):
        # Assert requests.get calls
        dispatch = dispatchRequest()
        payload={'terminus:user_key':"mykey"}
        
        json_data = dispatch.sendRequestByAction('http://localhost:6363/',const.CONNECT,payload)
